...
def visualize_training_episode(env, agent=None, max_steps=1000, use_rl=True, use_iqn=True):
    ...
    # 打印实际使用的机器人数量
    actual_num_robots = len(env.robots)
    if args.yaml and args.num_robots is not None and actual_num_robots != args.num_robots:
        print(f"ℹ️  注意: YAML中有 {actual_num_robots} 个机器人，已使用全部（请求了 {args.num_robots} 个）")
    elif args.yaml:
        print(f"ℹ️  从YAML加载了 {actual_num_robots} 个机器人")
