...
def dashboard(eval_schedule,i):
    """显示评估进度和当前时间"""
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # 🔥 新增：获取当前时间
    print(f"\n======== eval schedule ======== [{current_time}]")  # 🔥 新增时间显示
...
def run_experiment(eval_schedules):
    ...
    print(f"📊 总测试场景数: {len(eval_schedules['num_episodes'])}")
    print(f"🔄 每个场景测试回合数: {eval_schedules['num_episodes']}")
    print("="*60)
    ...
    for idx,count in enumerate(eval_schedules["num_episodes"]):
        dashboard(eval_schedules,idx)  # 这里会显示当前时间
        ...
        # 🔥 新增：显示每个场景的开始时间
        scene_start_time = datetime.now().strftime("%H:%M:%S")
        print(f"🕒 场景 {idx+1} 开始时间: {scene_start_time}")

        for i in range(count):
            # 🔥 新增：每10个回合显示一次进度和时间
            if i % 10 == 0:
                current_time = datetime.now().strftime("%H:%M:%S")
                print(f"📈 进度: 场景 {idx+1}, 回合 {i}/{count} [{current_time}]")
        ...
        # 🔥 新增：显示每个场景的结束时间
        scene_end_time = datetime.now().strftime("%H:%M:%S")
        print(f"✅ 场景 {idx+1} 完成时间: {scene_end_time}")
    ...
    total_duration = datetime.now() - datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
    print(f"🎉 实验完成时间: {end_time}")
    ...
    print(f"📄 数据文件: exp_results.json")
    print(f"🖼️  图表文件: success_rate.png, time.png, energy.png")
