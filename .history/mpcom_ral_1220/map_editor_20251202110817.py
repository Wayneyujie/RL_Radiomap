...
class MapEditor:
    ...
    def create_buttons(self):
        ...
        # Add obstacle button
        ax_add = plt.axes([0.05, button_y, button_width, button_height])
        self.btn_add = Button(ax_add, 'Add Obstacle\n(Drag)')
        self.btn_add.on_clicked(self.enable_draw_mode)
        
        # Set start position button
        ax_start = plt.axes([0.05 + button_spacing, button_y, button_width, button_height])
        self.btn_start = Button(ax_start, 'Set Start')
        self.btn_start.on_clicked(self.enable_set_start)
        
        # Set goal position button
        ax_goal = plt.axes([0.05 + 2*button_spacing, button_y, button_width, button_height])
        self.btn_goal = Button(ax_goal, 'Set Goal')
        self.btn_goal.on_clicked(self.enable_set_goal)
        
        # Clear all button
        ax_clear = plt.axes([0.05 + 3*button_spacing, button_y, button_width, button_height])
        self.btn_clear = Button(ax_clear, 'Clear All')
        self.btn_clear.on_clicked(self.clear_all)
        
        # Save YAML button
        ax_save = plt.axes([0.05 + 4*button_spacing, button_y, button_width, button_height])
        self.btn_save = Button(ax_save, 'Save YAML')
        self.btn_save.on_clicked(self.save_yaml)
        
        # World size input boxes
        ax_width_label = plt.axes([0.05 + 5*button_spacing, button_y + 0.03, 0.08, 0.02])
        ax_width_label.axis('off')
        ax_width_label.text(0.5, 0.5, 'Width:', ha='center', va='center', fontsize=9)
        
        ax_width = plt.axes([0.05 + 5*button_spacing, button_y, 0.08, button_height])
        self.text_width = TextBox(ax_width, '', initial=str(self.world_width))
        self.text_width.on_submit(self.update_width)
        
        ax_height_label = plt.axes([0.05 + 6*button_spacing, button_y + 0.03, 0.08, 0.02])
        ax_height_label.axis('off')
        ax_height_label.text(0.5, 0.5, 'Height:', ha='center', va='center', fontsize=9)
        
        ax_height = plt.axes([0.05 + 6*button_spacing, button_y, 0.08, button_height])
    ...
    def enable_set_start(self, event):
        ...
        print("Click on canvas to set robot start position")
    ...
    def enable_set_goal(self, event):
        ...
        self.setting_start = False
        self.drawing_mode = False
        print("Click on canvas to set robot goal position")
    ...
    def on_mouse_press(self, event):
        ...
        # Check if right click (delete obstacle)
        ...
        if event.button == 1:
            if self.drawing_mode:
                ...
                # Set start position
                self.robot_start = [event.xdata, event.ydata]
                self.setting_start = False
                self.update_display()
                print(f"Start position set to: ({event.xdata:.2f}, {event.ydata:.2f})")
            elif self.setting_goal:
                # Set goal position
                self.robot_goal = [event.xdata, event.ydata]
                self.setting_goal = False
                self.update_display()
                print(f"Goal position set to: ({event.xdata:.2f}, {event.ydata:.2f})")
    ...
    def update_display(self):
        ...
        self.ax.set_title('Map Editor - Drag to draw obstacles, Right-click to delete', fontsize=14, fontweight='bold')
        ...
        self.ax.scatter(self.robot_start[0], self.robot_start[1], 
                       s=300, c='green', marker='o', 
                       edgecolors='darkgreen', linewidth=2, zorder=10,
                       label='Start Position')
        
        # Draw robot goal position
        self.ax.scatter(self.robot_goal[0], self.robot_goal[1], 
                       s=300, c='red', marker='*', 
                       edgecolors='darkred', linewidth=2, zorder=10,
                       label='Goal Position')
        
        # Display information
        info_text = f'Obstacles: {len(self.obstacles)}\nWorld Size: {self.world_width:.1f}m × {self.world_height:.1f}m'
