...
class MarineNavEnv2(gym.Env):
    ...
    def __init__(self, seed:int=0, schedule:dict=None, yaml_config_path:str=None):
        ...
        if yaml_config_path is not None and os.path.exists(yaml_config_path):
            self.load_yaml_config(yaml_config_path)
    ...
    def load_yaml_config(self, yaml_path: str):
        ...
        try:
            ...
            if 'world' in self.yaml_config:
                world = self.yaml_config['world']
                if 'width' in world:
                    self.width = float(world['width'])
                if 'height' in world:
                    self.height = float(world['height'])
            
            print(f"✓ Loaded YAML config from {yaml_path}")
            print(f"  World size: {self.width}m × {self.height}m")
    ...
    def load_obstacles_from_yaml(self):
        ...
        obstacles = []
        if self.yaml_config is None or 'obstacles' not in self.yaml_config:
            return obstacles
        
        obstacle_groups = self.yaml_config['obstacles']
        if not obstacle_groups or len(obstacle_groups) == 0:
            return obstacles
        
        for group in obstacle_groups:
            if group.get('type') == 'obstacle_polygon' and 'distribute' in group:
                ...
                for state, shape in zip(states, shapes):
                    if len(state) >= 2 and len(shape) >= 3:
                        # Calculate obstacle center and size from shape
                        ...
                        # state is the position where shape's [0,0] vertex is placed
                        state_x = float(state[0])
                        state_y = float(state[1])
                        center_x = state_x + width / 2
                        center_y = state_y + height / 2
                        
                        # Use the larger dimension as radius for circular approximation
                        r = max(width, height) / 2
                        
                        obs = Obstacle(center_x, center_y, r)
                        obstacles.append(obs)
        
        return obstacles

    def load_robots_from_yaml(self):
        """Load robot start/goal positions from YAML configuration
        Returns: list of tuples (start, goal, cooperative) for each robot
        If YAML has fewer robots than num_cooperative, generate additional robots randomly
        """
        robot_configs = []
        if self.yaml_config is None or 'robots' not in self.yaml_config:
            return robot_configs
        
        robots_data = self.yaml_config['robots']
        
        # Check if it's a single robot or list of robots
        if isinstance(robots_data, list):
            # Multiple robots
            for robot_data in robots_data:
                if 'state' in robot_data and 'goal' in robot_data:
                    start = np.array([float(robot_data['state'][0]), float(robot_data['state'][1])])
                    goal = np.array([float(robot_data['goal'][0]), float(robot_data['goal'][1])])
                    cooperative = robot_data.get('cooperative', True)
                    robot_configs.append((start, goal, cooperative))
        else:
            # Single robot or old format
            if 'state' in robots_data and 'goal' in robots_data:
                start = np.array([float(robots_data['state'][0]), float(robots_data['state'][1])])
                goal = np.array([float(robots_data['goal'][0]), float(robots_data['goal'][1])])
                cooperative = robots_data.get('cooperative', True)
                robot_configs.append((start, goal, cooperative))
        
        # If we have fewer robots than needed, generate additional ones randomly
        total_robots_needed = self.num_cooperative + self.num_non_cooperative
        if len(robot_configs) < total_robots_needed:
            num_missing = total_robots_needed - len(robot_configs)
            print(f"YAML has {len(robot_configs)} robots, generating {num_missing} additional robots randomly")
            
            # Generate additional robots
            for i in range(num_missing):
                ...
                while iteration > 0:
                    ...
                    # Check if this start/goal is valid
                    valid = True
                    if np.linalg.norm(goal-start) < self.min_start_goal_dis:
                        valid = False
                    else:
                        # Check against existing robot configs
                        for existing_start, existing_goal, _ in robot_configs:
                            if np.linalg.norm(existing_start - start) <= self.clear_r:
                                valid = False
                                break
                            if np.linalg.norm(existing_goal - goal) <= self.clear_r:
                                valid = False
                                break
                    
                    if valid:
                        # Determine if cooperative based on how many we have
                        cooperative = len(robot_configs) < self.num_cooperative
                        robot_configs.append((start, goal, cooperative))
                        break
    ...
    def reset(self):
        ...
        # Load from YAML if available, otherwise use random generation
        use_yaml = (self.yaml_config is not None)
        
        if use_yaml:
            # Load obstacles from YAML
            self.obstacles = self.load_obstacles_from_yaml()
            print(f"Loaded {len(self.obstacles)} obstacles from YAML")
            
            # Load robots from YAML
            robot_configs = self.load_robots_from_yaml()
            
            if len(robot_configs) > 0:
                # Use YAML robot configurations
                # If YAML has more robots than requested, use all YAML robots
                # Otherwise, generate additional robots if needed
                total_needed = self.num_cooperative + self.num_non_cooperative
                
                # If YAML has more robots, use all of them and update num_cooperative
                if len(robot_configs) > total_needed:
                    print(f"YAML has {len(robot_configs)} robots, using all of them (requested {total_needed})")
                    # Update num_cooperative to match YAML robot count
                    # Assume all YAML robots are cooperative for simplicity
                    self.num_cooperative = len(robot_configs)
                    self.num_non_cooperative = 0
                    total_needed = len(robot_configs)
                
                while len(robot_configs) < total_needed:
                    # Generate additional robots if needed
                    iteration = 500
                    while iteration > 0:
                        start = self.rd.uniform(low = 2.0*np.ones(2), high = np.array([self.width-2.0,self.height-2.0]))
                        goal = self.rd.uniform(low = 2.0*np.ones(2), high = np.array([self.width-2.0,self.height-2.0]))
                ...
                # Use YAML robot configurations (use all if YAML has more than requested)
                for start, goal, cooperative in robot_configs[:total_needed]:
                    rob = robot.Robot(cooperative=cooperative)
                    rob.start = start
                    rob.goal = goal
                    self.reset_robot(rob)
                    self.robots.append(rob)
                print(f"Loaded {len(self.robots)} robots from YAML")
            else:
                # Fall back to random generation if no robots in YAML
                use_yaml = False
                print("No robots found in YAML, using random generation")
        ...
        if not use_yaml:
            ...
            # If using YAML, still need to initialize core_centers (even if empty)
            centers = None
            for core in self.cores:
                if centers is None:
                    centers = np.array([[core.x,core.y]])
