...
with open("coordinates5.txt", "r") as file:
    for line in file:
        ...
        data = np.load(f"./radio_maps/radio_case{testcase}.npy")  # 加载数据

        if not (x > 64 or (x < 59 and y > 8 and x > 48 and y < 19) or y > 20.32 or x < 44.55 or y < 2): # x better >46
...
#     if -50 < point[0] <= -30 and len(selected_points) < 3:
...
# for point in intensity_values:
#     if -75 < point[0] <= -60 and len(selected_points) < 7:
#         if point not in unique_points:
#             selected_points.append(point)
#             unique_points.add(point)
          
# for point in intensity_values:
#     if -90 < point[0] <= -75 and len(selected_points) < 10:
#         if point not in unique_points:
#             selected_points.append(point)
#             unique_points.add(point)
...
start_x=10
start_y=2
data = np.load(f"./radio_maps/radio_case{testcase}.npy")
