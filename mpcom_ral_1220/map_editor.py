...
class MapEditor:
    ...
    def load_yaml(self, filename='map_editor_output.yaml'):
        ...
        try:
            ...
            # Load robot positions - 支持多个机器人
            self.robots = []
            if 'robots' in yaml_data:
                robots_data = yaml_data['robots']
                ...
                if isinstance(robots_data, list):
                    ...
                    for robot_data in robots_data:
                        if 'state' in robot_data and 'goal' in robot_data:
                            start = [float(robot_data['state'][0]), float(robot_data['state'][1])]
                            goal = [float(robot_data['goal'][0]), float(robot_data['goal'][1])]
                    ...
                    if 'state' in robots_data and 'goal' in robots_data:
                        start = [float(robots_data['state'][0]), float(robots_data['state'][1])]
                        goal = [float(robots_data['goal'][0]), float(robots_data['goal'][1])]
            ...
            for i, robot in enumerate(self.robots):
                print(f"   - Robot {i+1}: Start({robot['start'][0]:.2f}, {robot['start'][1]:.2f}), Goal({robot['goal'][0]:.2f}, {robot['goal'][1]:.2f})")
            return True
    ...
    def save_yaml(self, event):
        ...
        for robot in self.robots:
            robot_data = {
                'type': 'robot_acker',
                'number': 1,
                'state': [self._format_number(robot['start'][0]), 
                         self._format_number(robot['start'][1]), 
                         0.5, 0],
                'shape': [0.322, 0.22, 0.2, 0.22],
                'goal': [self._format_number(robot['goal'][0]), 
                        self._format_number(robot['goal'][1]), 
                        0],
                'vel_type': 'steer',
                'vel_min': [-1, -1],
                'vel_max': [1, 1],
                ...
                'arrive_mode': 'state',
                'edgecolor': 'b',
                'cooperative': robot.get('cooperative', True)
            }
            yaml_data['robots'].append(robot_data)
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                ...
                # Write robots section - 多个机器人
                f.write('robots:\n')
                for i, robot_data in enumerate(yaml_data['robots']):
                    f.write(f"  # Robot {i+1}\n")
                    f.write(f"  - type: '{robot_data['type']}'\n")
                    f.write(f"    number: {robot_data['number']}\n")
                    # Format state array
                    state_list = robot_data['state']
                    ...
                    f.write(f"    shape: {robot_data['shape']}\n")
                    # Format goal array
                    goal_list = robot_data['goal']
                    goal_str = '[' + ', '.join(str(self._format_number(x)) for x in goal_list) + ']'
                    f.write(f"    goal: {goal_str}  # Target position\n")
                    f.write(f"    vel_type: '{robot_data['vel_type']}'\n")
                    f.write(f"    vel_min: {robot_data['vel_min']}\n")
                    f.write(f"    vel_max: {robot_data['vel_max']}\n")
                    f.write(f"    psi_limit: {robot_data['psi_limit']}\n")
                    f.write(f"    arrive_mode: '{robot_data['arrive_mode']}'\n")
                    f.write(f"    edgecolor: '{robot_data['edgecolor']}'\n")
                    f.write(f"    cooperative: {robot_data.get('cooperative', True)}\n")
                    if i < len(yaml_data['robots']) - 1:
                        f.write('\n')
                f.write('\n')
...
def main():
    """Main function"""
    print("=" * 60)
    print("Map Editor - Multi-Robot Support")
    print("=" * 60)
    print("Usage:")
    print("1. Click 'Add Obstacle' button, then drag on canvas to draw rectangles")
    print("2. Right-click on obstacles to delete them")
    print("3. Click 'Add Robot' to add a new robot")
    print("4. Use 'Robot' text box to select which robot to edit (1, 2, 3, ...)")
    print("5. Click 'Set Start' or 'Set Goal' button, then click on canvas to set position")
    print("6. Right-click on robot start/goal to delete that robot")
    print("7. Click 'Delete Robot' to delete the currently selected robot")
    print("8. Adjust world size (width and height)")
    print("9. Click 'Save YAML' button to save map configuration")
    print("=" * 60)
