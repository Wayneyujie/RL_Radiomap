...
class MPC:
    def __init__(self, car_tuple, ref_path, receding=5, sample_time=0.1, iter_num=4, enable_reverse=False, rda_obstacle=False, **kwargs) -> None:
        # rededing = 10
        ...
        '''
        # self.last_reached_index = -1  # 初始设置为-1，表示尚未达到任何关键点
        # self.passed_indices = []  # 存储已经经过的关键点索引的列表
    ...
    def control(self, state,  ref_speed=5, obstacle_list=[],**kwargs):
        ...
        if self.cur_index == len(cur_ref_path) - 1:
            # # 判断是否经过关键点
            # if self.cur_index not in self.passed_indices:
            #     self.passed_indices.append(self.cur_index)
            #     print("****** 已经通过关键点", self.cur_index + 1, "******")
        ...
        return u_opt_array[:, 0:1], info  #info包含了优化信息
